...
export async function collectKimiResult(h, { directory, artworkId, dataRoot, report }) {
  ...
  try {
    ...
    const heading = original.doc.elements
      .filter(
    ...
    const select = async (id) => {
      ...
      await canvas.locator(selector).filter({ visible: true }).first().click({ force: true });
    };
    ...
    const text = canvas.locator('[data-c2a-label="Text"] textarea');
    await text.fill('Golden edit check');
    await text.blur();
    await expect
      .poll(async () =>
    ...
    await select(heading.id);
    const move = canvas.locator('.ed-a1a2-transform-y');
    await move.fill(String(heading.bounds[1] + 2));
    await move.blur();
    await expect
      .poll(async () => (await snapshot()).elements.find((e) => e.id === heading.id)?.bounds[1])
      .toBe(heading.bounds[1] + 2);
    edited = await snapshot();
    await saved(edited);
    await exportAndRecord('edited-move', 'png');
    await select(photo.id);
    for (const [i, edge] of ['l', 't', 'r', 'b'].entries())
      await canvas.locator(`.ed-a1a2-crop-${edge}`).fill(String(i === 0 ? 0.12 : 0));
    await canvas.locator('.ed-a1a2-crop-apply').click();
    await expect
    ...
    report.editing = {
      ...
      imageId: photo.id,
      method: 'native text/move/crop UI controls, autosave, close/reopen and YAML roundtrip',
    };
  } finally {
