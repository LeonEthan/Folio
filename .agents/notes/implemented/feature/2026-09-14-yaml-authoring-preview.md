...
创作预览 now watches the YAML artwork projection of the open 设计会话 (`design.yaml`, `pages/canvas.yaml`, needed `media/`) and shows a read-only uncommitted view. Idle 显式导入 saves the already-previewed YAML snapshot as 当前画稿 after validation. Leftover Kimi/open-kimi `.pptd` is not a watch or import source. Invalid or unstable files keep the last valid preview and never clear the current artwork. This is [#39](https://github.com/LeonEthan/Geon/issues/39) only; turn collection, skills, and fingerprint cleanup remain later slices.
...
Deterministic tests cover YAML observation dependencies, leftover PPTD refusal without an importable identity, source-path resolution of `design.yaml` while a leftover `.pptd` is present, observation fallback watches, retained invalid/unstable previews, and explicit import of the displayed snapshot. Native verification fixtures now write YAML. Turn collection, human-save publish, no-artifact detection, skill rewrite, PPTD-E\* rename, and source-manifest unpin remain [#38](https://github.com/LeonEthan/Geon/issues/38)–[#41](https://github.com/LeonEthan/Geon/issues/41). Blind eval is [#42](https://github.com/LeonEthan/Geon/issues/42). PR: [LeonEthan/Geon#43](https://github.com/LeonEthan/Geon/pull/43).
