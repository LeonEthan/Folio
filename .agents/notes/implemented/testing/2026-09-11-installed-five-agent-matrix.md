...
retaining its isolated profiles, endpoint, logs and teardown. The scoped macOS matrix
is complete under the 2026-09-12 automatic-save/public-reminder target, including
the supplemental real-image criterion. Retired strict generation-proof gaps and
failed historical rounds remain recorded; completion uses the specific installed
combinations below, not a claim that every behavior passed on one latest package. No Agent is selected on the user's
...
for non-secret configuration and cases. Across the recorded batches, five real
generate calls and two edits have produced seven outputs, with native image
reading and selected replacement/commit evidence. Two original
...
repeated on one latest package, or replace a failed historical result. The supplemental selected-image
real MCP output/read and replacement/commit/export/reopen evidence is now complete.
Its successful paid outputs and zero-paid recovery steps remain separate from the
original harness failures; see the live TODO for exact rounds and receipts. The scoped
...
| 7. Documentation and checks                           | This owning matrix and affected instructions retain outcomes, failures and check records.                                                                                                                                                                                                                                                                                                        | Completion relies on the scoped native evidence and explicit human verdicts, not documentation checks alone; public release is separate.                                                                                                                                                                                                                                                        |
...
The first current-contract Codex attempt,
`folio-t28-codex-current-A7d1Y8`, used that same `47c0808d` package and
the frozen script SHA256
`2084422f50eda7a6923ec1c7d9ad29482a64dde09958efd369374fafa3a2283a`.
It completed installed-package boot and natural Codex capability creation, then
created and opened the design session. The pre-save read-only probe immediately
threw `No native Bento canvas` before a `folio-design://` webContents appeared,
rather than returning a pending poll result. No human save, reminder, current-PPTD
or collector step ran, and no synthetic provider request was started. The retained
`result.json` is failed with `cleanupErrors=[]`; all teardown phases completed, the
isolated data root is absent and all nine recorded owned processes are absent.
...
was not read or changed. At that time real-user registration remained
unexecuted; it was authorized and executed on 2026-09-12 (see the
[plugin note](../../proposed/architecture/2026-09-12-kimi-reminder-plugin.md)).
