...
已本地集成 14/29 项。T14 主动预览已在独立分支完成，等待主分支集成；T06、T15、T25 分别推进显式重提交与新候选生产清退、文件变化自动刷新、Git/IDE 入口精简。共同修改同一文件时串行接线；其余 Agent hook、实时预览和完整旅程要求保持不变。

以上十四项组合（`9944930`）已通过主分支完整 `pnpm check`、格式及文档检查，包括类型、lint、测试及公开/平台边界检查。前一批联合检查修正了新增 Electron 多实例探针的两处变量遮蔽；该修正不改变探针行为。T07 集成保留了 onboarding 与普通文件回执两组翻译键；T05 合并保留了历史文件入口和 T08 已删除缩略图的采集流程。T08 真实读图证据与其主分支集成的运行时代码一致，仅根计划进度不同；其他单项真实运行证据仍限于各自记录的源码组合，不据此宣称最终安装包或全部 Agent 已验收。
...
用户已确认[整体收敛](../simplification/2026-09-11-design-workflow-convergence.zh.md)，没有阻碍拆分的产品问题。旧 PPTD 自动提升、结果卡和专用缩略图分别已由 T01/T07/T08 删除，新候选生产仍待 T06 退役；旧文件、素材、提交事实继续保留。以下边界保持不变，已交付部分见上方集成记录：
