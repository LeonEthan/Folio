...
| T21 / #23 | 本次集成（源 `01ac4f9`） | [持续创作与运行时切换已交付](../../implemented/bug-fix/2026-09-11-design-continuation-runtime-identity.md)：恢复沿用既有 Session 派发与历史，读取证据绑定实际启动和 provider；取消、失败、冲突保留当前稿与草稿。真实 Pi/Claude 桌面验证重开、明确继续和菜单切换，重新取得读取与生成事实，退出清理通过；主线联合检查通过。使用合成模型响应和预备缓存，安装包冷启动及人工视觉验收仍由 T28/T29 完成 |
...
已本地验收并集成 23/29 项。T21 的运行时重建隔离、失败/取消恢复和实际 Agent 切换已通过单项原生旅程及主线联合检查。T22 的[成功提交后返回当前稿修复与交互证据](../../implemented/bug-fix/2026-09-11-current-canvas-after-commit.md)已交付：修复集成于 `c3ec872`，原生桌面完整旅程和中文输入法等技术核验已有记录；人工完整旅程及宽窄窗口的视觉判断仍待完成，不计入上述完成数。T28 准备最终安装包矩阵，T29 已独立开始完整旅程、性能与发布说明的准备；最终组合的包内验证仍待完成。共同修改同一文件时协调接线，所有原始验收要求保持不变。
