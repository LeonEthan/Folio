...
最新组合的真实 Electron 开发构建另通过既有桌面烟测：3 个场景、18 个步骤，覆盖引导、会话停止/删除及 worktree/终端释放，外部 ACP 使用合成服务。初次运行因实际目录中的技能文件让合成仓库不再干净而触发原有保留保护；仅调整该测试仓库的内置技能忽略规则后通过，PPTD、素材和自定义技能仍计为未提交文件。生产保稿规则未改；该证据不代替五种 Agent、完整画布只读矩阵或安装包验收。

## 已确认删减与剩余实施项

用户已确认[整体收敛](../simplification/2026-09-11-design-workflow-convergence.zh.md)，没有阻碍拆分的产品问题。旧 PPTD 自动提升已由 T01 删除，结果卡/缩略图及候选工作流仍待退役；旧文件、素材、提交事实继续保留。以下边界保持不变，已交付部分见上方集成记录：
