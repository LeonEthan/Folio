...
已本地集成 13/29 项。T05、T14、T25 分别推进 Pi 读写 hook、主动刷新只读 PPTD 预览、Git/IDE 入口精简；共同修改同一文件时串行接线。未完成的 hook、新候选生产清退、实时预览和完整旅程要求保持不变。

以上十三项组合（`6372da6`）已通过主分支完整 `pnpm check`、格式及文档检查，包括类型、lint、测试及公开/平台边界检查。前一批联合检查修正了新增 Electron 多实例探针的两处变量遮蔽；该修正不改变探针行为。T07 集成保留了 onboarding 与普通文件回执两组翻译键。T08 真实读图证据与其主分支集成的运行时代码一致，仅根计划进度不同；其他单项真实运行证据仍限于各自记录的源码组合，不据此宣称最终安装包或全部 Agent 已验收。
