...
**所有权表述。** store.ts 模块文档与 [design-bento README](../../../../packages/design-bento/README.md) 的 P1 节改为「单一提交者、两个调用方」：Electron 侧 CLI worker 经 stdin 转发 UI 请求，守护进程回合后 in-process 调用同一批导出；两者只靠内容定址状态（sha256 revisionId 对比调用方基线）协调，不共享锁、端口或进程所有权；失去基线的调用方得到 `DESIGN_CONFLICT` 且字节不会落地，守护进程据此转为候选而非覆盖。
