...
Folio 将成熟的 Lody coding-agent 工作台与 PPTD + Bento 可编辑平面设计能力结合，形成 Agent 自主使用的设计环境（agent-naive）。保留既有会话、工具、权限、恢复和 UI，设计服务仅补充格式转换、画稿及素材持久化和版本保护。P0–P2 已完成既定验收，P3–P6 尚未实施；本次收敛后续范围，新增当前画稿按需同步、工具 hook 与文件驱动的只读实时预览，取消独立素材库、历史版本与自动合并引擎的迁移要求。五种 Agent 的实际 hook 接入、反向转换与持续预览仍待验证，参考图附件等未决项单列；整份计划仍为 proposed，Spec 为 draft。
...
主要流程：现有输入框发起设计 → Agent 生成可编辑作品 → 选中元素对话修改或手工调整 → 保存及重开 → 导出。重新生成或冲突时查看当前稿与候选并显式采用/丢弃；普通创作不增加固定评审步骤。
...
相邻项目确有素材库：其素材面板提供预览、下载、插入画布、用作参考图和重新生成；`packages/orchestration/src/orchestrator.ts` 的 `listAssets` 从历史修订汇总已登记素材，不另建独立素材存储。Folio 已不迁入历史修订库，为这个入口重新引入汇总和长期保留机制不符合当前范围。取舍是放弃应用内跨作品查找和复用素材，保留当前画布的图片插入、替换、选中图片重新生成及文档所需素材持久化。
...
本轮按确认的修改方向修订当前方案和 P3–P6；P0–P2 的已实施记录保留历史语义，不作为后续扩大迁移范围的授权。新增的[按需同步与 hook 决策](2026-09-10-design-sync-hooks.zh.md)负责说明反向转换、文件边界与五种接入的验证要求；产品意图同步到 [Spec](../../../../specs/graphic-design-platform.zh.md)。
...
完整基线/当前/新稿三方对比及属性/元素自动合并退出迁移计划；保留最小候选查看和显式采用/丢弃。独立素材库、历史版本库、作品目录导入和品牌资源管理不进入任何迁移阶段；模板市场、多人协作等也不作为迁移延期待办。参考文件和常用尺寸沿用文件入口与轻量预设。

## 文件实时预览补充（2026-09-11）

确认增加 Agent/外部编辑器实际文件变化驱动的 PPTD → BentoDoc 实时预览，按打开的消费者共享订阅，复用既有 intake/importer 和 Bento 渲染。文件变化只触发读取和展示，不提交当前稿、不判断回合完成、不改变 Agent 草稿基线；详细源码依据、快照限制和失败行为见[实时预览决策](2026-09-11-pptd-live-preview.zh.md)。

用户确认首期创作预览只读：Agent 运行中不采用，完成后沿用现有正式提交/候选采用；没有运行回合时，当前会话的外部 PPTD 修改可显式导入为候选并采用。当前稿仍可独立手工编辑；切换到预览不丢未保存编辑，也不改变读取 hook 的输入对象。应用的反向同步投影不进入创作监听/产物采集，杜绝预览引发再次提交的循环。

本补充不恢复任何已取消的素材库、历史版本、三方合并或目录导入计划；原三项未决问题继续保留。本轮仅修改设计文档。
...
| 主对话                                                            | 改造               | 保留正文、工具展开、输入体验，增加作品卡、当前稿/候选查看和选区上下文               |
...
| 设置                                                              | 改造               | 通用、外观、快捷键、Agents、图像连接、保存、关于；MCP/Role 放高级项 |
| token/上下文与耗时                                                | 保留必要信息       | 图像调用与费用仅展示可观测数据；未知不显示零费用                             |
| Git diff、提交、分支、worktree、合并                              | 从设计主流程移除   | 按消费者决定保留/隐藏/删除；不为设计改造重写或整体删除 Git 基础设施                  |
...
| 设计候选                                                          | 迁入必要能力       | 当前稿/候选查看、显式采用/拒绝、版本保护；不迁入自动合并与历史版本                        |
| 正式导出                                                          | 迁入               | PNG/JPEG、透明背景行为和实际像素尺寸明确；从已保存当前稿生成                 |
| 参考文件与尺寸预设 | 复用或轻量适配 | 沿用文件/附件，保留静态常用尺寸；不新建品牌资源、项目素材管理或存储 |
...
画布顶部放作品名、尺寸、保存状态和导出；选择元素后显示属性。图层和图片属性按需展开，避免同时固定项目侧栏、对话、图层、画布、属性五个面板。新建设计可以从一句话、参考图或尺寸预设开始，不先显示庞大模板库。
...
flowchart TD
  ...
  PROJECTION --> CLI
  CLI --> CANDIDATE[独立创作草稿和候选]
  EXTERNAL[外部编辑器] --> CANDIDATE
  CANDIDATE --> WATCH[P3：按消费者监听创作文件]
  WATCH --> INTAKE[复用 intakeAuthoring / importPptd]
  INTAKE --> PREVIEW[独立只读 Bento 预览]
  CANDIDATE -->|正式采集或显式导入候选| DESIGN
...
- **人工编辑不能丢**：派发快照保留历史事实；P3 新增 Agent 读取当前稿的基线。最终提交原子检查相应基线，过期或重生成时保留独立候选；不自动合并，不因出现文件而停止 Agent。
...
| P3 持续创作       | 当前稿可读、hook、文件实时预览与最小候选处理 | P2                     | 多轮迭代、人工/Agent 协同和恢复           |
...
P3.1 处理反向转换与读取/写入一致性，P3.7 复用已有正向转换建立独立实时预览。两者先对齐投影/草稿来源边界即可分别推进，实时预览不以五种 hook 全部完成为前置；选区与已有设计修改仍依赖 P3.1 的相应基线能力。详细职责见[同步决策](2026-09-10-design-sync-hooks.zh.md)与[实时预览决策](2026-09-11-pptd-live-preview.zh.md)，不迁入上游完整编排服务。

**工作项**

- P3.1a 在设计能力包实现 BentoDoc → PPTD，建立支持字段和素材映射，验证往返可编辑语义、稳定 ID/顺序/样式。明确不支持项并报错，不静默扁平化。
- P3.1b 设计服务复用画布通用 snapshot/flush，按 Agent 读取请求同步当前稿投影与素材；成功读取才建立作品/草稿/版本基线。投影与 Agent 工作草稿隔离，保护未提交修改，派发 manifest 不被回合内读取改写，投影不进入 Agent 新产物采集。
- P3.1c 接入共享同步与版本检查，通过五种 Agent 的薄 hook/extension 适配受控读写；缺失/过期读取基线拒绝写入，错误不伪装成功。最终提交保留独立原子版本检查，不依赖 hooks 全覆盖。逐一验证实际 runtime、ACP 启动和工具路径，不覆盖用户全局配置。
- P3.1d 将选区的稳定元素 ID、作品和读取基线绑定到输入，支持局部修改及整体重新生成；失效选区不自动换目标。沿用会话输入、附件和引用机制。
- P3.2 保留当前画布的图片插入、替换及选中图片重新生成，复用编辑器和现有 Agent 回合入口；修改不得破坏当前稿和待处理候选所需素材。不新增独立素材库、跨作品素材浏览/复用或独立素材管理存储。
- P3.3 复用现有结果卡、候选存储和采用/丢弃，补齐当前稿/候选查看和必要状态。采用时再次校验版本，重复操作幂等；取消完整三方对比与属性/元素自动合并要求。
- P3.4 未决候选和已有持久草稿在重开后仍可处理，明确未保存修改的恢复边界；不实现历史缩略图、历史版本打开或修订恢复，不新增 checkpoint 引擎。
- P3.5 验证并补齐 Lody 既有失败/取消后的显式继续、Agent 切换与作品上下文恢复；保持 provider 会话引用分离，不另建生命周期和恢复系统，不在重启时自动重试付费请求。
- P3.6 修订 graphic-design skill：保留格式/能力/文件/工具合同及设计建议，取消固定 inspect/draft/review 顺序、inspect-once、自选分析方式禁令和强制工具/评审次数。Agent 决定调查、自检和完成方式；应用只做结构校验。正式导出仍使用固定 Bento 渲染器。
- P3.7a 复用 `collectAuthoring → intakeAuthoring → importPptd` 和已有预览载荷，补齐依赖闭包、采集前后内容复核及固定素材字节；不新建转换器，不将稳定观测等同作者完成事务。
- P3.7b 为打开的实时预览建立受限文件订阅，复用/适配 Lody watcher 的共享生命周期，支持缺失路径创建、rename、依赖调整、断开释放和重新打开主动核对；不触发代码协作全目录扫描。连续事件合并、摘要去重、有限重采，旧任务和旧订阅结果不得发布。
- P3.7c 以已有 Bento 资源呈现独立只读创作预览及更新/无效/监听失败状态，保留上一有效结果和未保存当前稿；预览不推进版本或参与产物完成，应用投影与缓存不进入监听/采集循环。
- P3.7d 接入已确认的采用边界：运行中不采用；回合后按现有正式采集/候选流程处理，已提交结果不重复采用；无运行回合的当前会话外部文件可显式导入候选。采用绑定已查看快照并重验当前稿版本，保护未保存编辑，不新增目录导入或中途自动提交。

P2-A3（OSS composer 把参考图作为回合附件）**不是本阶段工作项**。P3.2 处理画布实际引用素材，回合附件处理 prompt 图片块及本地身份；保持独立待定，不随素材库取消，也不自动并入 P3。

**模块**：`packages/design-authoring` 转换、CLI 设计服务/回合输入与采集、现有 Agent 接入、选区桥接、会话输入及编辑器图片操作。另复用现有 watcher 底层和预览载荷，按精确目标扩展订阅能力，不开启 Code Collab 索引或 All Changes。优先扩展已有深层模块，不新建通用 hook 平台或设计 runtime。

**验收**：手工修改后，Agent 的实际读取得到对应最新快照；往返保持可编辑语义。未读取、读取失败、过期读取、并行工具调用或 hook 缺席均不能导致当前稿被陈旧结果覆盖。再次读取不覆盖 Agent 草稿，也不使旧草稿自动换基线；应用投影不被收为新候选。Agent 改标题期间用户改背景时允许产生候选，两份内容均保留，不承诺自动合并。查看候选期间再次保存后，旧采用请求不能覆盖新稿。继续与重开沿用已有会话，缺素材和恢复失败不伪报成功。Agent/脚本/外部编辑器分步写入时只读预览更新，暂时无效保留上一有效结果且不污染当前稿；重复字节不重转、旧结果不倒灌。多消费者共用监听，最后关闭释放；无消费者回合继续正常采集。反向投影不触发循环，采用不能偷换为随后变化的源内容。

### P4：统一为设计工作台

**工作项**

- P4.1 基于现有首页输入、会话和侧面板适配创作、编辑、候选处理、导出及设置。沿用组件、主题、导航和标签页；概念图中的素材库、历史版本/恢复、独立作品库与新建流程不作为实现要求。
- P4.2 侧栏呈现项目与设计会话，使用作品缩略图和生成/待回应状态；调整画布默认宽度、专注模式、窄窗口视图切换和属性面板；在同一侧面板区分可编辑当前稿与只读创作预览，切换保留未保存编辑和撤销状态。
- P4.3 将重新生成、替换图片、调整风格做成携带目标和普通提示词的上下文动作，沿用 Agent 派发；清楚呈现只读预览、更新受阻与显式采用状态，运行中不提供采用；保留草稿、附件、引用、工具折叠、流式滚动及输入法行为。
- P4.4 整理 Agents/图像连接/保存设置，Role/MCP 放高级项；改造首个设计引导。尺寸使用轻量静态预设，参考资料沿用文件/附件入口；不新建设计资源、品牌或项目素材管理系统。
...
**验收**：首页输入 → 创作 → 手工/对话修改 → 必要的候选处理 → 保存重开 → 导出的交互连续；能查看独立实时预览，等待有效文件/监听失败不会伪装为当前稿已更新；键盘、中文输入法、弹窗焦点、深浅主题和窄窗口可用。由人工实际操作评价体验，不能只凭截图通过。新呈现组件和关键状态按仓库规则补 Storybook。
...
**验收**：设计项目无需 Git/GitHub 可完成主流程，已停止无消费者的开发后台任务；保留的 Agent 文件/工具/诊断/权限/恢复链路仍可用。不提供作品目录导入；旧程序数据不受清理影响。边界检查证明未引入相邻项目 Web/mobile 或私有运行依赖。
...
- P6.3 固定合成海报、信息图、长图样例，验证 Bento/PPTD 往返语义、手工编辑可读、草稿不被同步覆盖、投影不被误收集；覆盖基线缺失/过期、并行和最终提交版本变化。复用既有测试设施，不建立第二套验证引擎。
- P6.4 验证普通用户安装、无 CLI 状态、图像服务错误、退出重开、磁盘写入失败、候选恢复；测量加载/编辑/保存/导出及内存表现，依据实测和目标机器确定可接受范围，补用户文档及发布说明。
- P6.5 验证安装包文件监听与实时预览：分步写入、素材同路径替换、原子 rename、依赖增删/暂缺、持续写入与资源预算、监听遗漏/恢复、多消费者及断开释放、旧结果迟到、无消费者后台回合。验证外部编辑与显式候选采用、预览切换保护未保存当前稿、源变化不偷换采用对象、hook 投影不会进入循环；使用现有确定性设施，平台监听另留实机证据。

**验收**：安装包内走完完整旅程，自动检查与人工视觉/编辑体验分别记录；每个宣称支持的平台、Agent 和 hook 路径均有对应证据。首发不要求应用自动安装外部 Agent，也不新增自动更新系统。打包通过不等于签名/发布完成，实际发布单独执行。

### 执行纪律、检查与拆分

- 依赖顺序为 P0 → P1 → P2 → P3 → P4 → P5 → P6；这是阶段验收依赖，不限制 P3.7 正向预览与 P3.1 反向同步在文件来源边界确定后分别推进。资源打包探针、现有 UI 适配与文档随所属能力同时完成，不全部拖到最后。这里没有创建执行任务或委派 Agent。
- 每个 Pn.x 是可拆分工作包，具体 PR 大小由真实差异决定。先按运行链交付完整切片，避免连续多批只落空接口的 PR；社区贡献仍遵循现有尺寸/指派规则。
- 每个改动做一次最贴近风险的检查：格式转换/版本检查用确定性测试，跨模块生命周期用集成检查，保存/退出/导出用实际桌面旅程。迁入已有有效测试，不建立重复测试框架。
...
以下三项未因本轮收敛而自动裁定或合并到 P3：

| 待定项 | 保留的事实与下一步 |
| --- | --- |
| P2.7 #2b：未改动旧 PPTD 成为候选 | 原实施决定保留；结合投影/草稿及读取基线边界重新裁定，不直接改写旧行为 |
| P2-A3：OSS 参考图回合附件 | 独立待处理；取消素材库不取消附件需求，P3.2 也不吸收此项 |
| reference-pack 的非 PNG 能力 | 不扩写自制编解码器；核实派发边界实际可用的解码能力后选择规范化 PNG，或明确限制。CLI 持有字节不是解码能力 |
...
2026-09-09 补充了[五张 UI 概念图与提示词](../../../../output/imagegen/folio-ui-v1/README.md)，覆盖首页、对话画布、素材库、版本对比及设置。通过内置 imagegen 生成，以同一工作台图作为其他页面的视觉参考；合成案例与 Agent 状态只用于界面示意。已检查主要布局和页面覆盖，没有进行交互实现或验收，方案仍为 proposed。素材库页面/导航、历史版本列表与恢复承诺已取消，完整三方对比也不进入迁移范围；仅保留当前稿/候选查看的方向。旧图仅保留为历史生成记录，不能恢复已取消的入口或要求。
...
后文 P0–P2 是历史实施记录。涉及当时延期的完整三方对比、自动合并或固定 skill 流程时，以本轮 P3–P6 计划为后续目标；不据此宣称既有代码已经改造。原未决项仍保留。
...
**i18n 与 Storybook。** `locales/en.json`、`zh_CN.json` 各 41 键（`design.result.*`），组件一律 `t(key, defaultValue)`；[DesignTurnResultCard.stories.tsx](../../../../packages/components/src/stories/DesignTurnResultCard.stories.tsx) 12 个状态，只注入 store 的回答并渲染真实组件（文案与外观都在组件里）。
...
**已决：无法归属本轮的工程转为候选（#2b）。** 上一轮遗留在磁盘上的工程（其采集从未跑完）本轮既没写、也证明不了属于自己的，三种处理里用户选定了**保留为候选**，实现见上。另两条被否决的理由写在这里，免得日后重新讨论：**禁止提交**（本文件先前的现状）把用户没有别的方式看到的产出永久藏起来，而工作区不是用户会去看的地方；**接受回滚**（直接提交为画布）等于让一个没有证据表明属于本轮的工程覆盖当前画布，与草案第 4 条「无法证明安全的修改不能覆盖当前稿」直接冲突。代价是：只要工作区里的工程与画布**不同**（用户手工改过画布，或采用后又编辑过），一次「什么也没做」的回合就会把同一份工程再报成候选——候选按内容定址，因此不会多出第二份文件，用户采用或丢弃即了结（「已采用」是每次读出来的状态而非写下去的记录，见 P2.5 段，所以画布一旦离开那份候选，它就会重新显示成「待处理」）。
...
**发现三（未修，不并入 P3）。** OSS 桌面无法把参考图作为附件发给设计回合：附件区返回「Failed」芯片（`use-chat-landing-image-draft.ts:128` 要求托管方的 `authToken`），`hasBlockingImages` 随即阻止发送，`references[]` 因图片来自托管会话的图片库而始终为空。本次验收因此改以 prompt 内的绝对路径提供参考图，本地 Agent 从磁盘读取——这是当前 OSS 下唯一可用的路径。处理方式已澄清，见文末 P2-A3 节；不是「待 P3 自然带上」。
...
### P2-A3 回合参考图附件（2026-09-10，未实施，不并入 P3）

**状态：未实施。** 这不是 P2-A2 那种缺一条重载的热修。composer 上传、守护进程组 prompt 图片块、聊天气泡/缩略图，整条会话图片链路都绑在托管方 `/session-images` 与 `authToken` 上。P2.2 冻结已经能吃「已下载字节」；OSS 桌面没有对等的本地身份和读回，且不得发认证产品云请求。纸夹因此 Failed 并挡住发送。

**与 P3 的边界。** 独立素材库已从整个迁移计划移除；P3.2 仅保留当前画布图片的插入、替换和选中图片重新生成。P2-A3 是这一回合 prompt 的参考图附件。两者都会在本地读图，但身份、生命周期和消费者不同：附件进 `design-input/<turnId>/references` 与 ACP 图片块；画布引用图片进当前稿的 assets。新开窗口执行 P3 **不会**自然包含 P2-A3；把它并进 P3.2 会做出两套互相漂移的本地图片入口。

**待选做法（未裁定）。**

- **诚实缺席**（小）：本地没有会话图片能力时不提供会失败的纸夹，也不因 Failed 芯片挡住发送；参考图仍走 prompt 路径/技能。符合能力可观测。不兑现「发起时附参考图」。
- **本地会话图片存储**（独立切片）：纸夹可用，字节进 P2.2 清单和 ACP 图片块，重开仍能显示。按能力门控，不写 `if Electron`，不打产品云。这才补上 P2 用户故事里的参考图附件，体量中大，且必须与当前画布引用素材保持两种身份；不以独立素材库为前置条件。

进 P3 之前若只消掉坏交互，用诚实缺席；若要把 OSS 设计闭环做成可点纸夹的产品路径，单独立项做本地存储，不要挂在 P3 任务里。开工完整存储前读 [平台合同](../../../../packages/platform/AGENTS.md)。
