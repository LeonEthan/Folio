...
P2-A3（OSS composer 把参考图作为回合附件）**不是本阶段工作项**。新开窗口执行 P3 时不要把它并进 P3.2：素材库处理的是画布实际引用的素材，回合附件是 prompt 图片块及其本地身份。范围与待选做法见文末 P2-A3 节。
...
**与 P3 的边界。** P3.2 素材库是画布里实际引用的素材（上传/预览/插入/替换/重新生成）。P2-A3 是这一回合 prompt 的参考图附件。两者都会在本地读图，但身份、生命周期和消费者不同：附件进 `design-input/<turnId>/references` 与 ACP 图片块；素材库进当前稿的 assets。新开窗口执行 P3 **不会**自然包含 P2-A3；把它并进 P3.2 会做出两套互相漂移的本地图片入口。

**待选做法（未裁定）。**

- **诚实缺席**（小）：本地没有会话图片能力时不提供会失败的纸夹，也不因 Failed 芯片挡住发送；参考图仍走 prompt 路径/技能。符合能力可观测。不兑现「发起时附参考图」。
- **本地会话图片存储**（独立切片）：纸夹可用，字节进 P2.2 清单和 ACP 图片块，重开仍能显示。按能力门控，不写 `if Electron`，不打产品云。这才补上 P2 用户故事里的参考图附件，体量中大，且必须与日后素材库保持两种身份。
