...
Folio 将成熟的 Lody coding-agent 工作台与 PPTD + Bento 可编辑平面设计能力结合，形成 Agent 自主使用的设计环境（agent-naive）。保留既有会话、工具、权限、恢复和 UI，设计服务仅补充格式转换、画稿及素材持久化和版本保护。P0–P2 已完成既定验收，P3–P6 尚未实施；本次收敛后续范围，新增当前画稿按需同步和工具 hook，取消独立素材库、历史版本与自动合并引擎的迁移要求。五种 Agent 的实际 hook 接入和反向转换仍待验证，参考图附件等未决项单列；整份计划仍为 proposed，Spec 为 draft。
...
2026-09-10 修订：当前请求授权更新方案文档；P0–P2 已另获实施授权并完成，P3–P6 仍为计划。本节取代此前五步概要；仍为 proposed，不把方向性认可当作新 Spec 已逐项批准。产品意图单独记录在 [Spec 草案](../../../../specs/graphic-design-platform.zh.md)。已有 UI 概念图作为实施参考，不作为控件能力或交互验收依据。
...
| P3 持续创作       | 当前稿可读、反向转换、hook、选区与最小候选处理 | P2                     | 多轮迭代、人工/Agent 协同和恢复           |
...
先完成 P3.1 的转换和文件一致性切片，再接选区与图片操作。详细职责、工具覆盖和失败语义见[同步决策](2026-09-10-design-sync-hooks.zh.md)，不迁入上游完整编排服务。
...
**模块**：`packages/design-authoring` 转换、CLI 设计服务/回合输入与采集、现有 Agent 接入、选区桥接、会话输入及编辑器图片操作。优先扩展已有深层模块，不新建通用 hook 平台或设计 runtime。

**验收**：手工修改后，Agent 的实际读取得到对应最新快照；往返保持可编辑语义。未读取、读取失败、过期读取、并行工具调用或 hook 缺席均不能导致当前稿被陈旧结果覆盖。再次读取不覆盖 Agent 草稿，也不使旧草稿自动换基线；应用投影不被收为新候选。Agent 改标题期间用户改背景时允许产生候选，两份内容均保留，不承诺自动合并。查看候选期间再次保存后，旧采用请求不能覆盖新稿。继续与重开沿用已有会话，缺素材和恢复失败不伪报成功。
...
- P4.3 将重新生成、替换图片、调整风格做成携带目标和普通提示词的上下文动作，沿用 Agent 派发；保留草稿、附件、引用、工具折叠、流式滚动及输入法行为。
...
**验收**：首页输入 → 创作 → 手工/对话修改 → 必要的候选处理 → 保存重开 → 导出的交互连续；键盘、中文输入法、弹窗焦点、深浅主题和窄窗口可用。由人工实际操作评价体验，不能只凭截图通过。新呈现组件和关键状态按仓库规则补 Storybook。

### P5：按消费者清理开发功能

**工作项**

- P5.1 按“入口 → hook/订阅 → 后台任务 → 依赖包”检查 GitHub/PR/CI/代码审查等仅开发用途链路；删除确无消费者的部分，保留通用文件、进程、工具、诊断、权限和恢复能力。
