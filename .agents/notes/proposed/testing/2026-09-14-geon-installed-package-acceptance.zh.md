...
`~/Library/Preferences/dev.geon.app.plist`、`~/Library/Application Support/Geon`
和 `~/.geon` 状态。

### 视觉身份

- [ ] 挂载 `Geon-0.1.0-arm64.dmg`；卷名、背景图和 `.VolumeIcon.icns`
  应显示 **Geon** 品牌，而不是 Lody/Folio 水母或旧水下/极光素材。
- [ ] 将 `Geon.app` 拖入 `/Applications`；Finder 和 Launchpad 中的应用图标应为新的 Geon 身份。
- [ ] 启动安装后的应用；Dock 图标和菜单栏菜单应标记为 **Geon**，而非 Lody/Folio。
- [ ] 打开 **Geon → 关于 Geon**；关于面板应显示 **Geon** 产品名、版本 `0.1.0` 和 Geon 仓库链接。
- [ ] 首次启动后检查 `~/Library/Preferences/dev.geon.app.plist`；bundle identifier 应为 `dev.geon.app`，且无 Lody/Folio 默认值泄漏。
