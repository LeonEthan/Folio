...
`@molly-design/design-authoring` owns Molly Design's **YAML artwork snapshot** seam: fail-closed
...
- `migrate-two-file.ts` — explicit conversion of previous Molly Design YAML drafts to fresh output,
...
- `collect-authoring.ts` — secure snapshot collection (allowlist relpaths,
  symlink/hardlink/escape rejection); Molly Design admits exactly one entry,
...
- `scripts/reference-pack.mjs` — dependency-free Node port of the upstream Pillow
  ...
  PNG. Ported because neither Molly Design nor the migrated intake covers reference-image
...
```sh
corepack pnpm --filter @molly-design/design-authoring build      # manifest verify + matrix embed + skill bundle
corepack pnpm --filter @molly-design/design-authoring typecheck  # build first, then tsgo
corepack pnpm --filter @molly-design/design-authoring test       # build first, then vitest
```
...
after building. It admits only the former Molly Design `design.yaml` + `pages/canvas.yaml`
