  ...
  page.elements.forEach((element, i) => {
    ...
    switch (element.elementType) {
      ...
      case "table": {
        ...
        if (tableFill !== undefined) out.push({ ref: tableFill, path: elAt(".fill"), sourceId: element.elementId });
        ...
        if (Array.isArray(rows)) {
          rows.forEach((row, r) => {
            ...
            row.forEach((cell, c) => {
              ...
              if (ref !== undefined) {
                out.push({ ref, path: elAt(`.rows[${r}][${c}].fill`), sourceId: element.elementId });
              }
            });
          });
        }
        ...
        if (isRecord(style)) {
          ...
          if (Array.isArray((style as Record<string, unknown>).bodyStyles)) {
            ((style as Record<string, unknown>).bodyStyles as unknown[]).forEach((s, bi) => {
              ...
              if (ref !== undefined) {
                out.push({ ref, path: elAt(`.style.bodyStyles[${bi}].fill`), sourceId: element.elementId });
              }
            });
          }
        }
