...
import { MOLLY_ISSUES_URL } from '@/lib/lody-urls';
...
export function ErrorBoundaryFallback({
  ...
  const headline = isRawConvexServerError(error)
    ? t('errorBoundary.serverErrorSummary', 'The Molly Design backend returned a server error.')
    : report.summary;
      ...
      >
        <div className="flex items-start gap-2.5">
          ...
          <div className="min-w-0">
            <h2 className={cn('font-semibold text-foreground', isPage ? 'text-base' : 'text-sm')}>
              {t('errorBoundary.title', 'Molly Design hit an unexpected error')}
            </h2>
            <p className="mt-1 text-xs text-muted-foreground sm:text-sm">
              {automaticRetriesStopped
                ? t(
                    'errorBoundary.descriptionRetriesStopped',
                    'This error came back every time, so Molly Design stopped retrying on its own — the screen now stays put until you choose a step below.'
                  )
                : t(
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            ...
            {t('errorBoundary.reload', 'Reload Molly Design')}
          </Button>
        ...
        <div className="rounded-md border border-border/60 bg-muted/20 p-3">
          ...
          <ol className="mt-1.5 list-decimal space-y-1 pl-4 text-xs leading-5 text-muted-foreground">
            ...
            <li>
              {t(
                'errorBoundary.stepReload',
                'Reload Molly Design. Your synced work is not affected.'
              )}
            </li>
            <li>
              {t(
                'errorBoundary.stepReport',
                'Still broken? Copy the error details and report the problem on Molly Design’s issue tracker.'
              )}{' '}
              <button
                type="button"
        ...
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            {t(
              ...
              'This signs you out and deletes everything Molly Design stored on this device — local caches, offline copies, and preferences — then restarts the app. Work already synced to your account stays safe and downloads again after you sign in. Unsynced local drafts on this device are lost.'
            )}
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
