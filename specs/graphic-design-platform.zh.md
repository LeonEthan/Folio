...
静态单画布的编辑与渲染以已迁入并验证的 Bento 范围为上限，只做删减；必要的 Lody 接入适配和已经明确确认的自动回写、先读文件提醒 hook、文件实时预览按各自边界实施，不据此扩充产品。Agent 创作面是 Geon 自有的 YAML 投影，不以 PPTD 或外部 PPT 技能为现行格式合同，不提供 PPTD 互通。适用的第一性原理、复用与 Agent 自主性规则见根 [AGENTS.md](../AGENTS.md#design-principles-and-migration-scope)。
...
Agent 可调用已有 `geon_render_preview` 渲染 YAML 画稿投影，再用实际可用的图片读取工具查看并继续修改。应用不要求通过结果卡取图，也不把渲染成功等同模型已读取图片；预览工具缺席时，仅说明该能力不可用，不替 Agent 判定全部视觉 review 失败。可选 finalize 脚本不构成必经写入顺序或文件发布事务。
