 ...
 *
 * The render host is the running Molly Design desktop, which polls the daemon over the
...
afterEach(() => {
  if (previousDataRoot === undefined) delete process.env.LODY_DATA_DIR;
  else process.env.LODY_DATA_DIR = previousDataRoot;
