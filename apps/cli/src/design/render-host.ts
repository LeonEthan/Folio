 ...
 * `folio_render_preview` is not registered, and a preview asked for with no host
  ...
  enqueue(work: DesignRenderHostWork): Promise<DesignRenderPreviewOutcome> {
    if (!this.isConnected()) {
      return Promise.resolve({
        status: 'refused',
        error:
          'the Folio desktop is not running, so no preview can be rendered. Do not retry: tell the user to open Folio and run the preview again.',
      });
    }
