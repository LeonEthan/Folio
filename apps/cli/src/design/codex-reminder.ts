...
export function codexDesignReminderConfig(command: string, rawConfig?: string) {
  ...
  if (Object.keys(config).some((key) => key.startsWith('hooks.')))
    throw Error(
      'Molly Design Codex reminders require CODEX_CONFIG hook overrides under one nested hooks object; dotted hook overrides were preserved without launching'
    );
...
export function withCodexDesignReminder(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  if (!existsSync(hook))
    throw Error('Molly Design Codex design reminder is missing from the CLI bundle');
