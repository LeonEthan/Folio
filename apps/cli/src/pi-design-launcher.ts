...
const command = process.env.MOLLY_DESIGN_PI_COMMAND;
const extension = process.env.MOLLY_DESIGN_EXTENSION;
if (!command || !extension) throw Error('Missing Molly Design Pi launch context');
...
const mcpExtension = process.env.MOLLY_PI_MCP_EXTENSION;
const child = spawn(
  command,
  [
    '--extension',
