 ...
 *
 * `geon_generate_image` and `geon_edit_image` are registered only when the asking session is a design
 ...
 *
 * `geon_render_preview` (P2.4b) rides the same socket for the same reason, but
 * asks a different question: whether a Geon desktop is polling this machine. It
...
/**
 * Whether a Geon desktop is polling the daemon right now, which is the whole
...
export async function resolveRenderHost(
  ...
  if (!connected) {
    logger?.debug('render gate: no Geon desktop is polling this machine, so nothing can render');
  }
  return connected;
}
