 ...
 * asks a different question: whether a Molly Design desktop is polling this machine. It
...
/**
 * Whether a Molly Design desktop is polling the daemon right now, which is the whole
...
export async function resolveRenderHost(
  ...
  if (!connected) {
    logger?.debug(
      'render gate: no Molly Design desktop is polling this machine, so nothing can render'
    );
  }
  return connected;
}
