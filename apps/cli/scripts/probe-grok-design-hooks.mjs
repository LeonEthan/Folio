...
const runtime = process.env.FOLIO_PROBE_GROK;
assert.ok(runtime, 'Set FOLIO_PROBE_GROK to the official Grok 1.0.13 executable');
assert.match(execFileSync(runtime, ['--version'], { encoding: 'utf8' }), /^grok 1\.0\.13 /);
const root = await mkdtemp(path.join(tmpdir(), 'folio-grok-hooks-'));
