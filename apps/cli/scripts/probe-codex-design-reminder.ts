...
const executable = process.env.FOLIO_PROBE_CODEX;
if (!executable) throw Error('Set FOLIO_PROBE_CODEX to the managed Codex executable');
assert.equal(
  execFileSync(executable, ['--version'], { encoding: 'utf8' }).trim(),
  `codex-cli ${manifest.version}`
);
const root = await mkdtemp(path.join(tmpdir(), 'folio-codex-reminder-'));
