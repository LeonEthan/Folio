...
const claude = process.env.FOLIO_PROBE_CLAUDE;
if (!claude) throw Error('Set FOLIO_PROBE_CLAUDE to pinned native executable');
...
const provider = createServer(async (req, res) => {
  ...
  if (resubmitting) {
    ...
    } else if (resubmitCalls === 1) content = [tool('mcp__lody__folio_resubmit_draft', {}, 0)];
  ...
  console.log(
    ...
    body.tools?.map((t) => t.name).filter((n) => n.includes('folio'))
  );
