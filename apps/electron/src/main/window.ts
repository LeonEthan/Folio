...
async function showUnresponsiveDialog(window: BrowserWindow): Promise<void> {
  ...
  try {
    const result = await dialog.showMessageBox(window, {
      ...
      defaultId: 0,
      cancelId: 0,
      title: 'Molly Design is unresponsive',
      message: 'Molly Design is not responding.',
      detail:
...
function attachMainWindowDiagnostics(window: BrowserWindow, recoveryTarget: ReloadTarget): void {
  ...
  webContents.on('did-fail-load', (_event, ...args: unknown[]) => {
    ...
    if (isInRecovery(window)) return
    showRecovery({
      message: 'Molly Design could not load its window.',
  ...
  webContents.on('preload-error', (_event, preloadPath, error) => {
    ...
    if (isInRecovery(window)) return
    showRecovery({
      message: 'Molly Design preload script failed to initialize.',
  ...
  webContents.on('render-process-gone', (_event, details) => {
    ...
    if (isInRecovery(window)) return
    showRecovery({
      message: 'The Molly Design window crashed.',
